...
class API:
    ...
    def __init__(
        self, email: str, password: str, websession: ClientSession, country: str = "US"
    ) -> None:
    ...
    async def async_update_device_info(self) -> None:
        ...
        devices_resp = await self.request("post", "v2/app/get_devs_list")
        ...
        # Get stations/hubs
        try:
            stations_resp = await self.request("post", "v2/app/get_hub_list")
...
async def async_login(
    ...
    """
    if api is None:
        api = API(email, password, websession, country)
    try:
